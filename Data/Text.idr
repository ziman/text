...
record EncodedString : Encoding -> Type where
  EncS : (getBytes_ : Bytes) -> EncodedString e

-- Required because the autogenerated projection is not very useful.
||| Project the underlying byte buffer out of an EncodedString.
getBytes : EncodedString e -> Bytes
getBytes (EncS bs) = bs
...
foldl' pE f z bs with (pE bs)
  | Nothing     = z
  | Just (c, n) = foldl' pE f (f (S n) z c) (assert_smaller bs $ dropPrefix (S n) bs)
...
foldr' pE f z bs with (pE bs)
  | Nothing = z
  | Just (c, n) = f (S n) c (Delay $ foldr' pE f z (assert_smaller bs $ dropPrefix (S n) bs))
...
uncons {e = Enc pE eE} (EncS bs) with (pE bs)
  | Just (c, skip) = Just (c, EncS $ dropPrefix (S skip) bs)
  | Nothing        = Nothing
...
takeBytes n (EncS bs) = EncS (takePrefix n bs)

private
dropBytes : Nat -> EncodedString e -> EncodedString e
dropBytes n (EncS bs) = EncS (dropPrefix n bs)
...
cpointsToBytes pE (S n) k bs with (pE bs)
  | Nothing = k
  | Just (c, nbytes) = cpointsToBytes pE n (S nbytes + k) (dropPrefix (S nbytes) bs)

||| O(n). Extract the first N code points.
take : {e : Encoding} -> Nat -> EncodedString e -> EncodedString e
take {e = Enc pE _} n (EncS bs) = EncS (takePrefix (cpointsToBytes pE n 0 bs) bs)

||| O(n). Drop the first N code points.
drop : {e : Encoding} -> Nat -> EncodedString e -> EncodedString e
drop {e = Enc pE _} n (EncS bs) = EncS (dropPrefix (cpointsToBytes pE n 0 bs) bs)
