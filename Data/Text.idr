...
foldl' pE f z bs with (pE bs)
  | Nothing     = z
  | Just (c, n) = foldl' pE f (f (S n) z c) (assert_smaller bs $ dropBS (S n) bs)
...
foldr' pE f z bs with (pE bs)
  | Nothing = z
  | Just (c, n) = f (S n) c (Delay $ foldr' pE f z (assert_smaller bs $ dropBS (S n) bs))
...
||| Pack a list of code points into a string.
